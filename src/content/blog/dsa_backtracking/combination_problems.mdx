...
Let's say we have the candidates `[1, 2, 3]` and the target `4`.

In the first level of the decision tree, we can add `1`, `2`, or `3` to the current combination.
So we have three branches in the first level of the tree.

![Combination Decision Tree - First Layer](./assets/combination_decision_tree_a.png)

Adding only one number to the current combination is not enough to reach the target, so we need to keep adding numbers until we reach the target or exceed it.

Since the problem states that we can use the same number multiple times, we can add the same number again to the current combination,
which means that for each node in the decision tree we should run the algorithm again with the same candidates and the updated current combination.

![Combination Decision Tree](./assets/combination_decision_tree.png)

From the decision tree, we can note that we have two scenarios where we should stop the recursion:
- When we have a complete combination that sums to the target, which means that the sum of the current combination is equal to the target.
- When the sum of the current combination exceeds the target, in this case we can stop the recursion as well,
since we know that adding more numbers will only increase the sum.

To build and navigate through the decision tree, we must to keep track of three things:
...
combination that sums to the target, which means that the sum of the current combination is equal to the target.

When we reach this base
case, we can add the current combination to our results list.

```python
    # verify if we have a complete combination that sums to the target
...
```python
    # verify if the sum of the current combination exceeds the target
    if sum(candidate) > target:
        return  # stop the recursion
```
...
def combinationSum(candidates, target):
    ...
    return [list(combination) for combination in results]
